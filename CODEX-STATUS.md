...
### Package 3 — W42 D7 live validation

Restricted live execution to the D7 allowlist and bounded templates. Added seven single-page coverage probes; all other examples receive explicit shape-only reasons. Namespace discovery is read-only and its value is never retained. Run: `python scripts/check_examples.py --live --profile DEFAULT --region us-chicago-1 --report docs/validation-cli.json` in the pinned CLI environment. Results: {'shape-verified': 226, 'passed': 35}. Every attempted failure remains a failure. MCP run: `OCI_CLI_PROFILE=DEFAULT OCI_CONFIG_PROFILE=DEFAULT uv run --frozen --project runtime oci-readonly-smoke --live --region us-chicago-1`; all selected checks passed, sanitized report in docs/validation-mcp.json. Full suite 209 passed and strict linters green. No mutations, broadened retries or raw tenancy output retained.
...
### Package 6 — W44 four-arm comparison

Shipped reproducible, licensed snapshots from research/refs/adibirzu_oci-skills and research/refs/oracle_mcp (both API and Cloud tool descriptions), arm definitions with the source fairness rules, common offline matcher and seeded arm order, source-fence lint, guard exposure replay, token estimates and per-case no-plugin retrieval deltas. Command: `uv run --frozen --project runtime python scripts/eval/head_to_head.py --json evals/results/head-to-head.json --markdown docs/head-to-head.md`; `--refresh-snapshots` refreshes build-only upstream inputs.

Expanded both skill arms' fence scope to directly referenced documents; fixed one newly exposed unbounded limits example in the shared pitfalls reference. This pack: 37.5% routing, 0/40 negatives, 277/277 authored fences valid, 0/20 guard exposures, approximately 6,386 resident tokens. Every arm's raw results and asymmetries are published. Arm b's root references and oci_cli forwarding spelling are included; its prompt-injection hook exists but is not a mutation guard. c/d command validity and all behavioral task deltas are unmeasured. No competitor tools or tenancy commands executed. Full suite 213 passed and strict linters green. The four-arm offline package is complete; V19 and model-backed V27/live V28 remain red/unmeasured with owners named in the report.

### Package 7 — W45 publication docs and provenance

Rewrote README, audit, install, optional-MCP and foundation docs; added changelog and repository instructions. Published actual guard matrix and token counts, the prescribed planning floor, 15-tool contract, host guard limitations, named research gaps, unowned service operation counts, provenance and trademark wording. Added release_report.py and shipped historical denylist, upstream MCP smoke and certification-summary evidence so research-only numbers remain reproducible after installation. Historical denylist audit and current prefix replay are explicitly distinct.

Removed stale legacy-exemption guidance and repaired the old CLI snapshot link. Fixed two public-link extraction errors (quoted URLs and <slug> templates), with a regression test. Corrected nightly link run: all 175 URL/negative-fixture checks passed; docs/validation-links.json retains evidence. Commands: `uv run --frozen --project runtime python scripts/release_report.py`, `python scripts/ci/check_links.py`, and the package gate commands above. Full suite 214 passed; strict linters, document links and license hygiene green. The final validation matrix remains the next package; README does not claim release readiness. No tenancy operations.

### Package 8 — W46 final matrix and branch disposition

Executed all V1–V28 checks within their stated scope. docs/validation-matrix.md and JSON contain one dated row per gate with its command, result, limitations and an owner for every nonpassing row. Shipped `scripts/ci/release_gate.py`; reproduce with `uv run --frozen --project runtime python scripts/ci/release_gate.py --live --links --live-help`. Live flags authorize only bounded D7 reads/public documentation GETs; no agent, competitor executor, hosted workflow or issue creation is launched by this script.

Final matrix: **24 PASS, 1 FAIL, 2 PARTIAL, 1 UNAVAILABLE**. V19 is 37.5% description routing against ≥90% (evaluation/routing maintainers). V24 has a passing local required-flag renderer but the hosted Tuesday schedule/issue path was not executed (CI maintainers). V27 returned early-access unavailable and no model-backed substitute score was measured (Claude provider/evaluation maintainers). V28's requested four-arm offline comparison is complete; the original live/model-backed behavioral comparison remains unmeasured (evaluation maintainers). **The branch is ready for review, not release or merge.** No red result is hidden or softened into success.

Validation: full regression suite **216 passed**, one dependency deprecation warning; final pre-commit full run also passed 216 tests. All strict linters, template and manifests pass without baseline exemptions. Fresh installed CLI help validation passed; all 261 example templates validate offline. Final D7 run: 35 CLI examples passed, 226 remain shape-only, and namespace discovery passed as a separately recorded prerequisite. Selected MCP reads passed. Public-link checks and negative fixtures all passed. Distributed copy has 33 skills, references, hooks, MCP configuration and notices, with `find . -type l` empty before environment creation. Source/plugin and copied manifests pass strict Claude validation. Script/guard registries regenerate deterministically and have been rechecked after final source changes.

The initial literal `pytest -k parse` selected no tests. Renamed existing parser tests so that exact command now runs and passes all 80 cases; the final matrix records the corrected result and retains the initial selection failure in JSON. Namespace discovery failures now remain explicit failures instead of allowing dependent shape-only skips to obscure the prerequisite. The generated examples description now correctly includes inert mutation recipes. CI includes the negative hard gate and full offline eval gate; it intentionally remains red at V19.

Seven preceding package commits: a574a51, 7f9b607, 44df8ba, 7870154, 257f1f4, 3126f26, d87c7bf. This is the eighth package commit. No tenancy mutation, deployment, push or merge was performed. Branch remains v2-foundation. The pre-existing references/auth-modes.md edit and three oracle-autonomous-db .handoff-* files were preserved outside these commits.
