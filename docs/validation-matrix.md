...
| `uv run --frozen --project runtime pytest -q tests skills/oci-incident-triage/tests skills/oci-security-posture/tests skills/oci-sdk-patterns/tests` | PASS | PASS | 2026-09-10 | 436 tests passed before final attachment/state refinements; all 41 affected tests then passed. Post-merge regression run recorded separately in handoff validation. |
| `uv run --frozen --project runtime pytest -q tests/test_packaging.py tests/test_installer.py` | PASS | PASS | 2026-09-10 | Copy installation inspected: 37 skills, shared references, no authoring-only files or symlinks before runtime setup. |
...
| `uv run --frozen --project runtime python scripts/eval/run.py --json evals/results/offline.json` | PASS | PASS | 2026-09-10 | Recorded 37-skill semantic selection: 77/80 and 76/80, 4/4 overlap pairs in both trials. Current fingerprints verified; isolated description selection only. |
| `uv run --frozen --project runtime python evals/run_routing.py --negatives` | PASS | FAIL | 2026-09-10 | Seed 29 has 1/40 negative firings on excluded historical OCI-C; seed 17 has zero. Additional boundaries pass 12/12. Both trials retained without relabeling. Owner: evaluation/routing maintainers. |
