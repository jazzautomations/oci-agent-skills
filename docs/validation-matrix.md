...
| V14 | `uv run --frozen --project runtime pytest -q tests skills/oci-incident-triage/tests skills/oci-security-posture/tests skills/oci-sdk-patterns/tests` | PASS | 2026-09-10 | 453 tests passed in the full suite after the final application-capability clarification. Temporary runtime dependencies used UV_LINK_MODE=symlink; distributed payload checks require copies without symlinks. |
...
| V19 | `uv run --frozen --project runtime python scripts/eval/run.py --json evals/results/offline.json` | PASS | 2026-09-10 | Recorded 37-skill semantic selection after ownership and application-capability repairs: 80/80 and 80/80; 4/4 overlap pairs in both trials. Original prompts, labels, policy, model and seeds unchanged. Input-bound description selection only; no host task completion claim. |
