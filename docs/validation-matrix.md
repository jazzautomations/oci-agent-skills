...
| V25 | `CLI_PYTHON scripts/check_examples.py --live --profile DEFAULT --region us-chicago-1 --report docs/evidence/validation-cli.json` | FAIL | 2026-09-09 | Script sweep: 4 blocked_missing_prerequisite, 2 failed, 4 inert_not_executed, 27 passed, 2 ran, gap. Identity, governance, policy-input and profile-user defects corrected. Scoped triage still receives Cloud Guard 404 and Support 403; see docs/evidence/helper-diagnostics.json. Missing prerequisites and no-data coverage remain explicit. Owner: OCI operator / skill maintainers. |
