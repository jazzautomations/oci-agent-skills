...
| V14 | `uv run --frozen --project runtime pytest -q tests skills/oci-incident-triage/tests skills/oci-security-posture/tests skills/oci-sdk-patterns/tests` | PASS | 2026-09-10 | 453 tests passed in the full suite after ownership-contract and boundary updates. Temporary runtime dependencies used UV_LINK_MODE=symlink; distributed payload checks require copies without symlinks. |
...
| V19 | `uv run --frozen --project runtime python scripts/eval/run.py --json evals/results/offline.json` | PASS | 2026-09-10 | Recorded 37-skill semantic selection after explicit ownership repair: 79/80 and 79/80; 4/4 overlap pairs in both trials. Current fingerprints verified; isolated description selection only. |
| V20 | `uv run --frozen --project runtime python evals/run_routing.py --negatives` | PASS | 2026-09-10 | Zero negative firings in both 40-case trials; 31/31 synthetic boundaries pass. Original corpus, labels, policy, model and seeds unchanged. Pre-repair and development traces preserved. |
