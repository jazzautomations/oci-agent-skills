...
def replay(root_name, report_path, verifier):
    root = Path(root_name)
    _, tree = snapshot(root_name)
    ...
    process = subprocess.run([sys.executable, str(tree / 'scripts/eval' / verifier), str(tree / report_path)],
...
def verify_historical(report, verifier, root=ROOT):
    ...
    if record is None:
        return None
    if registry['commit'] != COMMIT or record['verifier'] != verifier:
        raise ValueError('Historical provenance changed')
    _, tree = snapshot(str(root))
    ...
    result = dict(replay(str(root), str(path), verifier))
    changes = changed_sources(report, root, tree)
    result.update(evidence_revision=COMMIT, current_sources=not changes, changed_sources=changes,
