...
def validate(path):
    ...
    return [finding(path, text[:m.start()].count('\n') + 1, name)
...
def main():
    ...
    if args.paths:
        selected = [file for p in args.paths for file in (p.rglob('*') if p.is_dir() else [p])]
    else:
        selected = [ROOT / p for p in subprocess.check_output(['git', 'ls-files', '-z'], cwd=ROOT).decode().split('\0') if p]
    return finish('secrets', [f for p in selected for f in validate(p)], args, len(selected))
