...
def arguments(description, live=False):
    ...
    parser.add_argument('--baseline', type=Path, help='Accept only exactly recorded legacy findings; never new findings.')
    if live:
...
def finish(name, findings, args, checked):
    ...
    baseline = json.loads(args.baseline.read_text()).get(name, []) if args.baseline else []
    # A stale exemption fails too, so a content owner must retire it when fixing a skill.
    ok = findings == baseline
    print(json.dumps({'validator': name, 'checked': checked, 'findings': findings,
                      'baseline_matches': ok if args.baseline else None, 'ok': ok}, indent=2))
