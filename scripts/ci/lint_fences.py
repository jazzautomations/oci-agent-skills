    ...
    for line, command in commands(path.read_text()):
        try:
            ...
            if not any(f in options for f in ('--help', '-h', '-?')):
                ...
                if row['verb'].startswith('list') and not {'--all', '--limit'} & options.keys():
                    raise ValueError('unbounded_list')
            if live:
                # Never execute the example: construct a fresh leaf-only --help argv.
                response = run([*leaf.split(), '--help'], capture_output=True, text=True, timeout=30)
            ...
            code = str(exc) if isinstance(exc, ValueError) and str(exc) in {'unknown_leaf', 'missing_required_flags', 'unbounded_list', 'live_help_failed', 'live_required_mismatch'} else 'invalid_command'
...
def main():
    ...
    return finish('fences', [f for p in selected for f in validate(p, args.live_help)], args, len(selected))
