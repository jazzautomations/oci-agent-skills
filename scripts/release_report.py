...
def main():
    ...
    historical=json.loads((ROOT/'docs/denylist-coverage-research.json').read_text())
    ...
    report={'skills':len(skills),'description_characters':desc,'description_tokens_estimate':(desc+3)//4,
        ...
        'oracle_historical':{'entries':historical['denylist_entries'],'destructive_allowed':historical['stats']['destructive_allowed'], 'mutating_allowed':historical['mutating_not_denied_count'], 'reads_denied':sum(r['kind']=='read' for r in leaves)-historical['stats']['read_allowed'], 'source':'research/data/denylist-coverage.json, shipped as docs/denylist-coverage-research.json; historical matching/classification differs from current prefix replay'},
        'oracle_current_prefix_replay':{'/'.join(k):v for k,v in sorted(current.items())},
        'historical_certification_counts':json.loads((ROOT/'docs/certification-coverage-research.json').read_text())['reported_counts'],
        'unowned_services':{name:services[name] for name in sorted(unowned)},
        'unowned_leaves':sum(services[name] for name in unowned),
        'live_cli_statuses':dict(Counter(r['status'] for r in json.loads((ROOT/'docs/validation-cli.json').read_text())['checks']))}
