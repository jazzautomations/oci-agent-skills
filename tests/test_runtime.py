...
@pytest.mark.parametrize(
    ...
    [
        "bad",
        "ocid1.compartment.oc1..bad'",
        "ocid1.compartment.oc1..bad\n",
        "ocid1.compartment.oc1..bad && x",
...
def test_oversized_page_does_not_silently_skip_rows():
    result = server.page_result(
        response([{"id": str(i)} for i in range(3)], "next"), "instances", 2
    )
...
@pytest.mark.parametrize(
    "error,kind",
    [
        (ValueError("secret"), "invalid_input"),
        (RuntimeError("secret"), "configuration_or_transport"),
        (oci.exceptions.ServiceError(403, "secret", {}, "secret"), "oci_service"),
    ],
)
...
def test_cost_filters_exact_compartment_region_and_rejects_unbounded_window(
    context, monkeypatch
):
    usage = Mock()
    ...
    assert {d.key: d.value for d in details.filter.dimensions} == {
